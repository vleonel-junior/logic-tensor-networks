...
\begin{frame}{Quantificateurs : la moyenne généralisée}
  ...
  \begin{itemize}
    \small
    \item pour $p = 1$ : les deux sont une moyenne simple,
...
\begin{frame}{De la base de connaissances au réseau}
  ...
  \begin{tikzpicture}[x=1cm, y=1cm]
    ...
    \node[align=center, text width=7.6cm] at (0,1.925) {
      ...
      \footnotesize $\mathrm{Sim}$ : $\exp(-\|u-v\|)$
    };
...
\begin{frame}{LTN contre baseline}
  ...
  rendu progressivement plus sélectif : $p=1\rightarrow2\rightarrow4
...
correspondent exactement aux changements de $p$ déjà présentés. Il faut
bien lire ces sauts : pour des degrés de vérité fixés, $\mathrm{pMean}$
augmente mécaniquement avec $p$, puisqu'il se rapproche du maximum. Une
grande partie de chaque saut vient donc du changement de la mesure
elle-même, et non d'un progrès soudain du classifieur : l'accuracy, elle,
ne fait pas de saut aux mêmes moments. Les niveaux de satisfaction ne sont
comparables qu'à $p$ égal.
}
